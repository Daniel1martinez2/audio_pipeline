#!/usr/bin/env python
"""
Complete Audio Pipeline with CNN Padding Approach
- No BPM normalization (preserves natural tempo)
- Variable-length mel spectrograms padded to uniform size
- FWOD vector computation from MIDI
- Optimized for CNN training

This pipeline skips time-stretching and uses padding instead,
preserving natural musical tempo characteristics.
"""
...
MAX_FRAMES_PER_BAR = 32  # Maximum frames per bar (for very slow songs)
...
class Example:
    ...
    mel: np.ndarray      # shape (N_MELS, MAX_FRAMES_PER_BAR) - padded
    fwod: np.ndarray     # shape (16,) - always 16 steps per bar
    mask: np.ndarray     # shape (MAX_FRAMES_PER_BAR,) - attention mask for padding
    meta: Dict[str, Any]
...
def calculate_frames_per_bar(bpm, sr, hop_length):
    ...
    frames_per_bar = max(MIN_FRAMES_PER_BAR, min(MAX_FRAMES_PER_BAR, frames_per_bar))
    
    log(f"BPM {bpm} → {frames_per_bar} frames per bar", 1)
    return frames_per_bar

def pad_mel_slice(mel_slice, target_frames=MAX_FRAMES_PER_BAR):
    """Pad or truncate mel slice to target frame count and create attention mask."""
    current_frames = mel_slice.shape[1]
    
    if current_frames < target_frames:
        # Pad with zeros
        padding = target_frames - current_frames
        padded_mel = np.pad(mel_slice, ((0, 0), (0, padding)), 'constant', constant_values=0)
        
        # Create attention mask (1 for real data, 0 for padding)
        mask = np.concatenate([np.ones(current_frames), np.zeros(padding)])
        
        log(f"Padded mel slice: {current_frames} → {target_frames} frames", 1)
    else:
        # Truncate if too long
        padded_mel = mel_slice[:, :target_frames]
        mask = np.ones(target_frames)
        
        if current_frames > target_frames:
            log(f"Truncated mel slice: {current_frames} → {target_frames} frames", 1)
    
    return padded_mel, mask
...
def slice_mel_and_fwod(
    ...
    """Generate Example objects for each bar, with padding for CNN compatibility."""
    ...
    try:
        ...
        for bar_idx, (f0, f1) in enumerate(bar_bounds):
            try:
                ...
                # Pad to uniform size
                padded_mel, attention_mask = pad_mel_slice(mel_slice)
                ...
                examples.append(Example(
                    ...
                    meta={
                        **meta, 
                        "bar_index": bar_idx,
                        "original_frames": mel_slice.shape[1],
...
def save_examples_to_npz(examples: List[Example], out_path: Path) -> None:
    ...
    try:
        ...
        if str(out_path) == '':
            ...
            out_path = script_dir / "output" / "mel_fwod_dataset_cnn.npz"
        ...
        masks = np.stack([ex.mask for ex in examples])
        ...
        log(f"Dataset shapes: mel={mels.shape}, fwod={fwods.shape}, mask={masks.shape}")
    except Exception as e:
...
def audio_to_train_dataset(
    ...
    log(f"Using CNN padding approach (max frames per bar: {MAX_FRAMES_PER_BAR})")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Audio processing pipeline with CNN padding approach",
    )
    ...
    parser.add_argument(
        "--output", 
        type=str, 
        default="mel_fwod_dataset_cnn.npz",
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("🎵 AUDIO PIPELINE WITH CNN PADDING APPROACH")
    print("="*70 + "\n")
    main()
